...
function buildHTML(data, logoSrc) {
  ...
  const logo = logoSrc
    ...
    : `<span style="font-size:24px;font-weight:900;color:#c85a1a;letter-spacing:2px">SYK</span>`;
  ...
  function sec(title, arr) {
    return `<div style="margin-bottom:10px">
      <div style="background:#c85a1a;color:#fff;font-weight:700;font-size:11.5px;padding:5px 10px;border-radius:3px 3px 0 0">${title}</div>
  ...
  return `<div style="
    ...
    <!-- هيدر -->
    <div style="display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:8px;margin-bottom:10px;border-bottom:2px solid #e8722a">
      <div style="font-size:10.5px;color:#444;line-height:1.9;text-align:left">
        <div>التاريخ: ${miladi}</div>
        <div>الموافق: ${hijri}</div>
        <div>رقم الخطاب: <b style="color:#c85a1a">${data.letter_number||''}</b></div>
      </div>
      <div>${logo}</div>
    </div>
...
async function generateLetterPDF(data, receiptDataURL = null) {
  ...
  try {
    ...
    if (receiptDataURL) {
      ...
      hd.innerHTML = `<div style="text-align:center;font-size:16px;font-weight:700;color:#c85a1a;border-bottom:2px solid #e8722a;padding-bottom:8px">صورة الإيصال</div>`;
